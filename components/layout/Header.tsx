...
function Logo({ className, variant = 'light' }: { className?: string; variant?: 'light' | 'dark' }) {
 ...
 variant === 'light' ? 'text-neutral-900' : 'text-neutral-900'
 )}>glyphatic</span>
 </Link>
 )
}
...
export function Header({ className }: HeaderProps) {
 ...
 className="text-neutral-900 hover:text-neutral-900/80 transition-colors focus-visible:outline-none"
 ...
 <Link
 href="/support"
 className="hidden md:inline-flex items-center justify-center rounded-full border border-black/20 px-5 py-2 text-[14px] font-medium text-neutral-900 transition-all hover:bg-black/10 hover:border-white/40"
 >
 Support
 </Link>
 ...
 className="rounded-md p-2 text-neutral-900 hover:bg-black/10 lg:hidden focus-visible:outline-none"
