        ...
        <FadeInView>
          ...
          >
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <p className="text-[1.25rem] md:text-[1.5rem] lg:text-[1.75rem] font-medium text-white leading-relaxed">
                  &ldquo;{testimonial.quote}&rdquo;
                </p>
                <div className="mt-8 flex items-center gap-4">
                  <div className="h-12 w-12 rounded-full bg-white/10 flex items-center justify-center text-white font-bold text-lg">
                    {testimonial.company.charAt(0)}
                  </div>
                  <div>
                    <p className="text-[15px] font-bold text-white">{testimonial.name}</p>
                    <p className="text-[14px] text-white/60">{testimonial.company} · {testimonial.industry}</p>
                  </div>
                </div>
              </motion.div>
            </AnimatePresence>
            ...
            <div className="absolute bottom-8 right-8 flex gap-2 z-10">
              <button
                ...
                className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/30 text-white hover:bg-white/10 transition-colors"
              >
              ...
              {TESTIMONIALS.map((_, i) => (
                <button
                  ...
                  className={`h-1.5 rounded-full transition-all ${
                    i === current ? 'w-8 bg-[#FA582D]' : 'w-4 bg-white/30 hover:bg-white/50'
                  }`}
                />
              ))}
            </div>
