...
export function TrustedByLogos({ logos = DEFAULT_LOGOS, title, className }: TrustedByLogosProps) {
  return (
    <div className={cn('py-8 bg-[#111111]', className)}>
      {title && (
        ...
        <div className="relative flex w-full overflow-hidden">
          ...
          <div className="animate-marquee flex w-max items-center justify-start gap-12 md:gap-16 lg:gap-24 py-4 pr-12 md:pr-16 lg:pr-24">
            {[...logos, ...logos, ...logos, ...logos].map((logo, i) => (
              ...
              >
                <span className="text-[13px] md:text-[14px] font-bold uppercase tracking-[0.15em] text-neutral-400 whitespace-nowrap select-none group-hover:text-white group-hover:drop-shadow-[0_0_12px_rgba(255,255,255,0.4)] transition-all duration-300">
                  {logo.name}
                </span>
              </div>
            ))}
